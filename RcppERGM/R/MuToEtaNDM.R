...
MuToEtaNDM <- function(mv, gy, freq, start = rep(0, ncol(gy)),
                    eps_f = 1e-8, eps_g = 1e-5, maxit = 300)
{
    MuToEtaNDM_(gy, freq, mv, start, eps_f, eps_g, maxit)
}
#' @export
PEtaNDM <- function(eta, gy, freq)
{
    PEtaNDM_(gy, freq, eta)
}
#' @export
PNDNDM <- function(eta, gy, freq)
{
    PNDNDM_(gy, freq, eta)
}
#' @export
EtaToMuNDM <- function(eta, gy, freq)
{
    EtaToMuNDM_(gy, freq, eta)
}

...
MuToEta <- function(mv, gy, freq, start = rep(0, ncol(gy)),
                    eps_f = 1e-8, eps_g = 1e-5, maxit = 300)
{
    MuToEta_(gy, freq, mv, start, eps_f, eps_g, maxit)
}
#' @export
PEta <- function(eta, gy, freq)
{
    PEta_(gy, freq, eta)
}
#' @export
PND <- function(eta, gy, freq)
{
    PND_(gy, freq, eta)
}
#' @export
EtaToMu <- function(eta, gy, freq)
{
    EtaToMu_(gy, freq, eta)
}

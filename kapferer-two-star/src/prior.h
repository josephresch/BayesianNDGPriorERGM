...
/*
	...
	For the Kapferer / esg model the sufficient statistic is
	(edges, kstar2, gwesp(0.25)) which mixes integer (edges, kstar2) and
	real-valued (gwesp) components, so the 3D hull hull3d_* is
...
typedef struct {
	...
	/* Current importance-sampling pool. Replaced wholesale at every
	   ...
	   the Kapferer / esg sufficient statistic is real-valued (kstar2,
	   gwesp). Parallel arrays of length ndg_m. */
	double *ndg_g1;         /* s1 values (edges)                           */
	double *ndg_g2;         /* s2 values (kstar2)                          */
	double *ndg_g3;         /* s3 values (gwesp)                           */
...
/* Create an adaptive NDG prior (3D).
     hull_eps   : numerical tolerance for the 3D hull (visibility,
                  ...
                  the default (1e-9). With Kapferer / esg statistics of

...
void hmc_algorithm_massmatrix(size_t iterations, SAMPLER2 *sampler, double *theta_cur, size_t nfrogs, double hmc_eps, size_t corr_type, size_t endterms, PRIOR *prior, char *results_filename, char *log_filename, char *time_filename, double target_accept, gsl_rng *rng)
{
	...
	/* Scratch buffers for handing the IS pool (adaptive NDG prior) from
	   ...
	   Kapferer / esg sufficient statistic (edges, kstar2,
	   gwesp(0.25)) is real-valued. */
	...
	/* Mass matrix -- Kapferer network (39 nodes, edges + kstar2 + gwesp),
	...
	double M[MAXSTATS*MAXSTATS] = {
	    177.308012024047969, 3064.390384769541015,  309.130641439082694,
	   3064.390384769541015, 54491.379835671294131, 5411.603185022944672,
	    309.130641439082694, 5411.603185022944672,   574.033817648792251
	};
	...
	fprintf(results_file,"theta_edges theta_kstar2 theta_gwesp\n");

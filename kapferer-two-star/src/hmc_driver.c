...
int main(int nargs, char **args)
{
	/*
		...
		args[1]  = data file
		args[2]  = model ("esg": edges + kstar(2) + gwesp(0.25))
		args[3]  = theta_cur[0]   (edges)
		args[4]  = theta_cur[1]   (kstar2)
		args[5]  = theta_cur[2]   (gwesp)
		args[6]  = sampler ("tnt")
		args[7]  = tnt_p
		args[8]  = grad_gap
	...
	*/
	...
	while(invalidmodel)
	{
		...
		{
			fprintf(stderr, "ERROR: Unsupported model (expected 'esg': edges + kstar2 + gwesp)\n");
	...
	{
		/* Caimo & Friel flat prior: N(0, diag(100)) on edges+kstar2+gwesp. */
		double mean[3] = {0.0, 0.0, 0.0};
		double var[3]  = {100.0, 100.0, 100.0};
		prior = prior_create_normal(3, mean, var);
		fprintf(stdout, "Prior: Normal(0, diag(100))  (edges+kstar2+gwesp)\n");
	}
	else if(strcmp(prior_type, "uniform") == 0)
	{
		/* Locked bounds:
		     ...
		     kstar2 ∈ [-2, 2]
		     gwesp  ∈ [-2, 5] */
		double lo[3] = {-8.0, -2.0, -2.0};
		double hi[3] = { 2.0,  2.0,  5.0};
		prior = prior_create_uniform(3, lo, hi);
		fprintf(stdout,
			"Prior: Uniform([-8,2] x [-2,2] x [-2,5])\n");
	}
	else if(strcmp(prior_type, "ndg") == 0)
	{
		/* Adaptive non-degeneracy prior in 3D (edges + kstar2 + gwesp).

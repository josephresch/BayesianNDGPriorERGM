...
/* GWESP decay parameter (alpha). Fixed for this build. */
...
*/
	typedef struct sampler2
	{
		...
		size_t 	nstats;
		double	*stats_orig;	/* stats (won't change) — real-valued for kstar2/gwesp */
		...
		/* model indicator */
		int model[MAXSTATS];									/* edge kstar2 gwesp ... */
...
/*
	get_twostars_change - change in 2-star count from toggling dyad at index. O(1).
*/
	double get_twostars_change(GRAPH *graph, size_t index);

/*
	get_gwesp - full-graph geometrically-weighted ESP statistic.

# 04_run_hmc_single.R
# Run the noisy HMC binary on Kapferer (edges + kstar(2) + gwesp(0.25)),
...
if (!file.exists("src/hmc")) {
  ...
  cat(sprintf("MPLE init: edges=%+.4f  kstar2=%+.4f  gwesp=%+.4f\n",
              mplecoef[1], mplecoef[2], mplecoef[3]))

  # Prior: "normal", "uniform", or "ndg"
  prior_type = "normal"
  ndg_file   = "../data/ndg_prior.txt"   # 3-column warm-start file (edges, kstar2, gwesp);
...
cat("\n--- HMC posterior summary (theta = (edges, kstar2, gwesp)) ---\n")
cat(sprintf("  prior: %s\n", prior_type))
...
summarize_column("theta_kstar2", hmc[[2]])
summarize_column("theta_gwesp",  hmc[[3]])
...
plot(hmc[[2]], type = "l", ylab = "theta_kstar2", xlab = "")
plot(hmc[[3]], type = "l", ylab = "theta_gwesp",  xlab = "iteration")
par(op)
dev.off()
...
# Storage
sim_modeled = matrix(NA, n_gof, 3, dimnames = list(NULL, c("edges", "kstar2", "gwesp")))
...
stat_names = c("edges", "kstar(2)", "gwesp(0.25)")
for (s in 1:3) {

...
#    Kapferer tailor-shop ~ edges + kstar(2) + gwesp(0.25, fixed=TRUE)
# under the Caimo-Friel prior N(0, diag(100)) on theta = (edges, kstar2, gwesp).
#
...
cat(sprintf("Observed stats: edges=%.0f kstar2=%.10f gwesp=%.10f\n",
...
# start: edge density → logit(p) for edges, 0 for kstar2/gwesp
...
write.csv(data.frame(param = c("edges", "kstar2", "gwesp"), value = mapcoef),

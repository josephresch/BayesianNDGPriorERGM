...
double get_logalphahmc_massmatrix(SAMPLER2 *sampler, double *theta, double *evol_theta, double *p, double *evol_p, double *Minv, double logzratio, double log_prior_ratio)
{
	...
	logmomentumratio = Minv[0]*(p[0]*p[0] - evol_p[0]*evol_p[0]) + Minv[3]*(p[1]*p[1] - evol_p[1]*evol_p[1]) + (Minv[1]+Minv[2])*(p[0]*p[1] - evol_p[0]*evol_p[1]);
...
void hmc_algorithm_massmatrix(size_t iterations, SAMPLER2 *sampler, double *theta_cur, size_t nfrogs, double hmc_eps, double target_accept, size_t corr_type, size_t endterms, PRIOR *prior, char *results_filename, char *log_filename, char *time_filename, gsl_rng *rng)
{
	...
	/* Scratch buffers for handing the IS pool (adaptive NDG prior) from
	   ...
	   arrays so the control flow stays uniform. */
	int *pool_s1 = malloc(sampler->ndraws * sizeof(int));
	int *pool_s2 = malloc(sampler->ndraws * sizeof(int));
	...
	/* setup mass matrix -- g7 network (7 nodes, edges+kstar(2) model) */
	double M[4] = {26.020460921843735,108.351743486973660,108.351743486973660,492.502589178357425};
	
	fprintf(stdout,"M: %.15lf %.15lf %.15lf %.15lf\n", M[0], M[1], M[2], M[3]);
	double Minv[4] = {0.0};
	...
	gsl_matrix_view M_gsl = gsl_matrix_view_array(M, 2, 2);
	local_cholesky_decomp(&M_gsl.matrix);
	gsl_vector *p_gsl = gsl_vector_calloc(2);
	gsl_vector *mu_vec = gsl_vector_calloc(2);
	...
	fprintf(log_file,"M: %.15lf %.15lf %.15lf %.15lf\n", M[0], M[1], M[2], M[3]);
	fprintf(log_file,"Minv: %.15lf %.15lf %.15lf %.15lf\n", Minv[0], Minv[1], Minv[2], Minv[3]);
	fprintf(log_file,"z_z'_correction: %zu\n", corr_type);
	fprintf(results_file,"theta_edge theta_2star\n");
  ...
  struct timespec end;
	...
	for(i = 0; i < iterations; ++i)
	{
		   ...
		   log p(theta_cur) while the pool is fresh at this point. */
		{
			...
			{
				pool_s1[j] = (int)sampler->samp_stats[j][0];
				pool_s2[j] = (int)sampler->samp_stats[j][1];
			}
			prior_ndg_set_pool(prior, pool_s1, pool_s2, (int)sampler->ndraws, evol_theta);
		...
		{
			/* step theta forward one full step */
			evol_theta[0] += hmc_eps*(Minv[0]*evol_p[0] + Minv[1]*evol_p[1]);
			evol_theta[1] += hmc_eps*(Minv[2]*evol_p[0] + Minv[3]*evol_p[1]);
		...
		/* step theta forward one final full step */
		evol_theta[0] += hmc_eps*(Minv[0]*evol_p[0] + Minv[1]*evol_p[1]);
		evol_theta[1] += hmc_eps*(Minv[2]*evol_p[0] + Minv[3]*evol_p[1]);
		...
		if(i < 1000)
		{
			...
			fprintf(stdout, "%zu eps: %.15lf epsbar: %.15lf nfrogs: %zu (%zu) theta: %.15lf %.15lf\n",i, hmc_eps, exp(logepsbar), nfrogs, (size_t)(int_time/exp(logepsbar)), theta_cur[0], theta_cur[1]);
		}
		if(i == 1000)
		{
			...
			if(nfrogs == 0) nfrogs=1;
			if(nfrogs > 50) { nfrogs = 50; hmc_eps = int_time/50.0; hmc_halfeps = 0.5*hmc_eps; }
			fprintf(stdout, "%zu eps: %.15lf epsbar: %.15lf nfrogs: %zu theta: %.15lf %.15lf\n",i, hmc_eps, hmc_eps, nfrogs, theta_cur[0], theta_cur[1]);
		}

		fprintf(results_file, "%+.16lf %+.16lf\n", theta_cur[0], theta_cur[1]);
		fflush(results_file);

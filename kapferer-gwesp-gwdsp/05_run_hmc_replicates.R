...
# MPLE coefficients for Kapferer ~ edges + gwesp(0.25) + gwdsp(0.25)
cat("Fitting MPLE...\n")
...
cat(sprintf("MPLE: edges=%+.4f  gwesp=%+.4f  gwdsp=%+.4f\n",
...
for (ii in seq_len(n_replicates)) {
  ...
  colnames(samples) = c("theta_edges", "theta_gwesp", "theta_gwdsp")
...
if (is.null(pooled) || nrow(pooled) == 0L) {
  ...
  colnames(pooled) = c("theta_edges", "theta_gwesp", "theta_gwdsp")
  ...
  sim_modeled = matrix(NA, n_gof, 3,
                       dimnames = list(NULL, c("edges", "gwesp", "gwdsp")))
  ...
  cat(sprintf("  Observed:  edges = %.0f,  gwesp = %.1f,  gwdsp = %.1f\n",
              obs_modeled[1], obs_modeled[2], obs_modeled[3]))
  stat_names_tbl = c("edges", "gwesp", "gwdsp")
  for (s in 1:3) {
  ...
  #   bot row: geodesic (3, spans 3 cols) | edges (4) | gwesp (5) | gwdsp (6)
  ...
  stat_labels = c("edges", "gwesp", "gwdsp")
  stat_ylims  = list(c(80, 280), c(80, 330), c(350, 1000))
  for (s in 1:3) {
...
for (pl in c("normal", "uniform")) {
  ...
  if (file.exists(fn)) {
    ...
    colnames(prior_data[[pl]]) = c("theta_edges", "theta_gwesp", "theta_gwdsp")
...
for (k in seq_along(ndg_files)) {
  ...
  colnames(prior_data[[label]]) = c("theta_edges", "theta_gwesp", "theta_gwdsp")

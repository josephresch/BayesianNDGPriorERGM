/*
	...
	(2) SAMPLER2 — a Metropolis-Hastings sampler with the Tie-No-Tie (TNT)
	    proposal, fitting the fixed 3-parameter "egd" ERGM
	        stats = (edges, gwesp(α=0.25), gwdsp(α=0.25)).
	...
	The "egd" model order is canonical throughout: stats[0]=edges,
	stats[1]=gwesp, stats[2]=gwdsp. α is the fixed macro ALPHA_GW{E,D}SP.
*/
...
/*
	...
	Does four things:
	  ...
	  (b) parse the model string (must be some permutation of "egd") into
	      a fixed-position model[] flag vector — the canonical storage
	      order is always [edges, gwesp, gwdsp] regardless of input order;
	  (c) allocate + compute stats_orig/stats_copy (incl. real-valued
	      gwesp/gwdsp), allocate changestats, build edges_orig/edges_copy
...
*/
	void tnt_attach2(SAMPLER2 *tntsampler, GRAPH *graph, char *model, double p)
	{
		...
		/* parse the model — canonical order: e, g, d */
		...
		for(i = 0; i < strlen(model); ++i)
		{
			switch(model[i])
			{
				...
				case 'd':
					...
					{
						fprintf(stderr, "ERROR: gwdsp selected more than once\n");
					}
					else
					{
		...
		{
			fprintf(stderr, "ERROR: this build requires model \"egd\" (edges + gwesp + gwdsp)\n");
		}
		...
		/* Canonical order: [edges, gwesp, gwdsp]. edges is exact; gwesp/gwdsp
		   are O(n * ndyads) full-graph recomputes done ONCE here at attach.
		...
		tntsampler->stats_orig[1] = tntsampler->stats_copy[1] = get_gwesp(graph, ALPHA_GWESP);
		tntsampler->stats_orig[2] = tntsampler->stats_copy[2] = get_gwdsp(graph, ALPHA_GWDSP);
	...
	void tnt_run2(SAMPLER2 *tntsampler, double *theta)
	{
		...
		while(1)
		{
			...
			for(k = 0; k < total_phase_iters; ++k)
			{
				...
				tntsampler->changestats[1] = get_gwesp_change(tntsampler->graph_copy, tntsampler->index, ALPHA_GWESP);
				tntsampler->changestats[2] = get_gwdsp_change(tntsampler->graph_copy, tntsampler->index, ALPHA_GWDSP);
...
/*
	get_gwdsp — Σ over ALL DYADS (i,j) of f(SP(i,j), α).
	Same as gwesp but without the edge filter. Since f(0)=0, dyads with zero
	shared partners drop out automatically, so in practice only dyads with
	non-trivial SP contribute.
*/
...
/*
	get_gwdsp_change — change in gwdsp from toggling dyad at index.

	Derivation. Every dyad (i,j) contributes f(SP(i,j)) regardless of edge status,
	so the edge filter of gwesp is dropped. Toggling (a,b) does not change SP(a,b).
	For dyad (a,k) with k != a,b: SP(a,k) changes by delta iff (k,b) is an edge
	(whether or not (a,k) itself is an edge). Symmetric for (b,k).

	    Delta gwdsp =
	        sum_{k != a,b, (k,b) edge} [f(SP(a,k) + delta) - f(SP(a,k))]
	      + sum_{k != a,b, (k,a) edge} [f(SP(b,k) + delta) - f(SP(b,k))]
*/
	double get_gwdsp_change(GRAPH *graph, size_t index, double alpha)
	{
		...
		{
			if(k == a || k == b) continue;
			/* Effect on dyad (a,k): flips iff (k,b) is an edge */
			if(get_dyad(graph, k, b))
			{
				size_t sp_ak = get_SP(graph, a, k);
				size_t sp_ak_new = (delta > 0) ? (sp_ak + 1) : (sp_ak - 1);
				dg += f_gw(alpha, sp_ak_new) - f_gw(alpha, sp_ak);
			}
			/* Effect on dyad (b,k): flips iff (k,a) is an edge */
			if(get_dyad(graph, k, a))
			{
				size_t sp_bk = get_SP(graph, b, k);
				size_t sp_bk_new = (delta > 0) ? (sp_bk + 1) : (sp_bk - 1);
				dg += f_gw(alpha, sp_bk_new) - f_gw(alpha, sp_bk);
...
*/
	void tnt_accept_change2(SAMPLER2 *tntsampler, size_t index, size_t edge_index)
	{
		...
		/* Fold the incremental change-stats into the running totals. After this
		   line, stats_copy exactly equals what get_gwesp/gwdsp would return on
